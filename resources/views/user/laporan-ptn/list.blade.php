            ...
            <div class="d-flex align-items-center gap-2 page-header-right-items-wrapper">
                <div class="dropdown filter-dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-end p-3">
                        ...
                        <!-- Filter Pembuat Laporan (nama pokja yang mengambil dari user_id) -->
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="filter-creator" name="filter-creator">
                                ...
                                @foreach($users as $user)
                                    @if($user->akses == 'User')
                                        <option value="{{ $user->id }}">{{ $user->pokja }}</option>
                                    @endif
                                @endforeach
                            </select>
                        </div>
                ...
                <div class="dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-end">
                        <a href="javascript:void(0);" class="dropdown-item" id="export-pdf" 
                            data-url="{{ route('user.laporan-ptn.export-pdf', ['pt_uuid' => $pt->uuid]) }}">
                            <i class="bi bi-filetype-pdf me-3"></i>
                            <span>PDF</span>
                        </a>
                        <div class="dropdown-divider"></div>
                        <a href="javascript:void(0);" class="dropdown-item" id="print-view"
                            data-url="{{ route('user.laporan-ptn.print', ['pt_uuid' => $pt->uuid]) }}">
                            <i class="bi bi-printer me-3"></i>
                            <span>Print</span>
                        </a>
                    </div>
                </div>
                <a href="{{ route('user.laporan-ptn.create', ['pt_uuid' => $pt->uuid])}}" class="btn btn-primary">
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover" id="laporanTable">
                            ...
                            <tbody>
                                ...
                                <tr class="single-item" data-user-id="{{ $item->user_id }}">
                                    <td>
                                    ...
                                    <td class="text-end">
                                        <div class="hstack gap-2 justify-content-end">
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-end">
                                                    <a href="{{ route('user.laporan-ptn.detail', $item->uuid) }}" class="dropdown-item">
                                                        <i class="feather-eye me-2"></i>Detail
                                                    </a>
                                                    
                                                    @if((Auth::id() == $item->user_id && $item->can_edit_delete) || Auth::user()->isAdmin() || Auth::user()->isDev())
                                                    <a href="{{ route('user.laporan-ptn.edit', $item->uuid) }}" class="dropdown-item">
                                                        <i class="feather-edit me-2"></i>Edit
                                                    </a>
                                                    @endif
                                                    
                                                    @if($item->dokumen_undangan)
                                                    <a href="{{ route('user.laporan-ptn.download-undangan', $item->uuid) }}" class="dropdown-item">
                                                        <i class="feather-download me-2"></i>Download Undangan
                                                    </a>
                                                    @endif
                                                    
                                                    @if($item->dokumen_notula)
                                                    <a href="{{ route('user.laporan-ptn.download-notula', $item->uuid) }}" class="dropdown-item">
                                                    ...
                                                    @if((Auth::id() == $item->user_id && $item->can_edit_delete) || Auth::user()->isAdmin() || Auth::user()->isDev())
                                                    <div class="dropdown-divider"></div>
                                                    <form action="{{ route('user.laporan-ptn.destroy', $item->uuid) }}" method="POST" class="d-inline delete-form">
                                                        @csrf
                                                        @method('DELETE')
                                    ...
                                    <td colspan="6" class="text-center">
                                        <div class="d-flex align-items-center justify-content-center" style="height: calc(100vh - 315px)">
                                            <div class="text-center">
                                                ...
                                                <a href="{{ route('user.laporan-ptn.create', ['pt_uuid' => $pt->uuid]) }}" class="avatar-text bg-soft-primary text-primary mx-auto" data-bs-toggle="tooltip" title="Buat Laporan Pertama">
