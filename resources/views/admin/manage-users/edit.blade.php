...
@section('title', 'Edit Pengguna ' . $user->pokja)
    ...
    <div class="col-lg-12">
        <div class="card stretch stretch-full">
            <div class="card-header">
                <h5 class="card-title">Edit Pengguna: {{ $user->pokja }}</h5>
            </div>
            <div class="card-body">
                <form action="{{ route('manage-users.update', $user->uuid) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        <label for="password" class="form-label">Password <small class="text-muted">(Kosongkan jika tidak ingin mengubah password)</small></label>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="akses" id="akses" class="form-select @error('akses') is-invalid @enderror" data-select2-selector="default" required>
                            @if(auth()->user()->isDev())
                                ...
                                <option value="Dev" {{ (old('akses', $user->akses) == 'Dev') ? 'selected' : '' }}>Developer</option>
                    ...
                    <div class="mb-3">
                        ...
                        @php
                            $userPokja = old('pokja', $user->pokja) ?? '';
                            // Jika masih dalam format array, ubah menjadi string dengan koma
                            if (is_array($userPokja)) {
                                $userPokja = implode(', ', $userPokja);
                            }
                        @endphp
                        <input type="text" name="pokja_input" id="pokja_input" class="form-control" value="{{ $userPokja }}">
                    </div>
